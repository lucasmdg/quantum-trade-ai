...
export function closePosition(pos: Position, exitPrice: number): TradeOrder | null {
  ...
  const pnlPercent = ((exitPrice - pos.entryPrice) / pos.entryPrice) * 100 * pos.leverage;
  ...
  const order = state.trades.find(t =>
    t.symbol === pos.symbol && t.status === "open" && t.entryPrice === pos.entryPrice
  );
  if (order) {
    order.status = "closed";
...
export function updatePositions(prices: Record<string, PriceTick>) {
  ...
  for (const [, pos] of positions) {
    ...
    if (pos.side === "long" && tick.price <= pos.stopLoss) {
      closePosition(pos, tick.price);
      continue;
    }
    if (pos.side === "long" && tick.price >= pos.takeProfit) {
      closePosition(pos, tick.price);
      continue;
    }
    if (pos.side === "short" && tick.price >= pos.stopLoss) {
      closePosition(pos, tick.price);
      continue;
    }
    if (pos.side === "short" && tick.price <= pos.takeProfit) {
      closePosition(pos, tick.price);
      continue;
    }
  }
