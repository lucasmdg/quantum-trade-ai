    ...
    <header className="sticky top-0 z-30 glass-panel border-b border-border px-4 lg:px-6 py-2">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Button size="sm" variant={running ? "red" : "green"} onClick={running ? stopEngine : startEngine} className="text-[11px]">
          ...
          <div className="flex items-center gap-1.5 text-[11px] text-dim">
            ...
            {([0.5, 1, 2, 5, 10] as const).map((s) => (
              <button key={s} onClick={() => setSimulationSpeed(s)}
                className={`px-1.5 py-0.5 rounded font-mono transition-colors ${simulationSpeed === s ? "text-accent bg-accent/10" : "hover:text-text"}`}>{s}x</button>
            ))}
          </div>
        </div>
        <div className="flex items-center gap-4 text-xs number-font">
          ...
          <div className="text-right hide-mobile">
            ...
            <div className={`font-semibold ${portfolio.currentDrawdown > 10 ? "text-red" : "text-dim"}`}>{portfolio.currentDrawdown.toFixed(1)}%</div>
          </div>
          <div className="text-right hide-mobile">
